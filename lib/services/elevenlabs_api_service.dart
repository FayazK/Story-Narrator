...
class ElevenlabsApiService {
  ...
  static const String _baseUrlV1 = 'https://api.elevenlabs.io/v1';
  static const String _baseUrlV2 = 'https://api.elevenlabs.io/v2';
  ...
  Future<void> initialize() async {
    ...
    if (_apiKey == null || _apiKey!.isEmpty) {
      throw Exception(
        'ElevenLabs API key not found. Please add it in the settings.',
      );
    }
  }
  ...
  Future<Map<String, dynamic>?> getUserInfo([String? apiKey]) async {
    try {
      ...
      final response = await _dio.get(
        '$_baseUrlV1/user',
        options: Options(headers: {'xi-api-key': key}),
      );
  ...
  /// Get user's voices (including custom and cloned voices) from ElevenLabs v2 API
  Future<List<Voice>?> getUserVoices([String? apiKey]) async {
    try {
      final key = await _getApiKey(apiKey);

      final response = await _dio.get(
        '$_baseUrlV2/voices', // Use v2 endpoint
        queryParameters: {
          'include_total_count': true, // As per API docs
        },
        options: Options(
          headers: {'xi-api-key': key, 'Content-Type': 'application/json'},
        ),
      ...
      if (response.statusCode == 200) {
        final responseData = response.data as Map<String, dynamic>;
        final List<dynamic> voicesData = responseData['voices'] ?? [];
        // Use the updated Voice.fromJson factory
        return voicesData.map((voice) => Voice.fromJson(voice)).toList();
      }

      return null;
    } catch (e) {
      debugPrint('Error fetching user voices: $e');
      return null;
    }
  }
  ...
  String getVoiceId(String? gender, String? voiceDescription) {
    ...
    } else if (description.contains('elderly') &&
            genderStr.contains('female') ||
  ...
  Future<Map<String, dynamic>?> getVoiceSettings(
    ...
    try {
      ...
      final response = await _dio.get(
        '$_baseUrlV1/voices/$voiceId/settings',
        options: Options(
          headers: {'xi-api-key': key, 'Content-Type': 'application/json'},
        ),
      );
  ...
  Future<String?> getVoiceAudioUrl(
    String voiceId,
    String text, [
    String? apiKey,
  ]) async {
    try {
      ...
      final response = await _dio.post(
        '$_baseUrlV1/voices/$voiceId/stream',
        options: Options(
          headers: {
            'xi-api-key': key,
  ...
  Future<String?> getVoicePreviewAudio(
    String voiceId,
    String previewText, [
    String? apiKey,
  ]) async {
  ...
  Future<File> generateVoiceover({
    ...
    try {
      ...
      final String finalVoiceId =
          voiceId ?? getVoiceId(gender, voiceDescription);

      // Get voice parameters based on language
      final voiceParams = getVoiceParameters(language ?? 'english', urduFlavor);

      final response = await _dio.post(
        '$_baseUrlV1/text-to-speech/$finalVoiceId/stream',
        options: Options(
          headers: {
            'Accept': 'audio/mpeg',
            'xi-api-key': key,
      ...
      if (response.statusCode != 200) {
        throw Exception(
          'Failed to generate voiceover: ${response.statusMessage}',
        );
      }
